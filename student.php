<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
        <title>T & P cell</title>
<link rel="stylesheet" href="css/bootstrap.min.css" type="text/css">
<link rel="stylesheet" href="css/bootstrap-theme.css" type="text/css">
<link rel="stylesheet" href="css/a.css" type="text/css">
  </head>
  <body>
<!-------------------Navbar start --------------------- -->  
<div class="container-fluid">

<div class="row a">
		<div class="col-lg-4 col-md-4 col-sm-4">
    	</div> <!-- div 1-->
    
    <div class="col-lg-4 col-md-4 col-sm-4">
    <div class="thumbnail b"><br>
 <center><h2 class="media-heading" style="color:cadetblue;">Search</h2></center> <hr>
		<form action="stuprocess.php" method="post">
    		<input class="form-control" type="text" placeholder="location" name="location"><br>
        	<input class="form-control" placeholder="course" type="text" name="course"><br>
            <input type="submit" value="Find" name="find" class="btn btn-danger btn-block"/>    
    	</form><br>
        
    </div><!-- thumbnail-->
    </div><!-- div 2-->
    
    	<div class="col-lg-4 col-md-4 col-sm-4">
    	</div><!-- div 3-->
        
</div><!-- row-->

</div><!-- container-->

    <!-- jQuery (necessary for Bootstrap's JavaScript plugins) -->
    <script src="jquery-1.12.3.min.js"></script>
    <!-- Include all compiled plugins (below), or include individual files as needed -->
    <script src="file:///C|/Users/dell 1/Desktop/Bootstarp/bootstrap-3.3.6-dist/js/bootstrap.min.js"></script>
  </body>
</html>
